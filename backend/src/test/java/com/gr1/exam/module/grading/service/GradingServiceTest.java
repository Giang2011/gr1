...
class GradingServiceTest {
    ...
    void grade_shouldRequireExactSetMatchForMultipleCorrect() {
        ...
        User user = User.builder().id(10).name("student").password("x").role(User.Role.STUDENT).build();
        Exam exam = Exam.builder().id(5).title("Exam").subject(subject).totalQuestions(1).duration(30).build();
        ExamSession session = ExamSession.builder().id(1).exam(exam).user(user).status(ExamSession.Status.SUBMITTED).build();
        ExamQuestion examQuestion = ExamQuestion.builder().id(201).examSession(session).question(question).orderIndex(1).build();

        UserAnswer userAnswer = UserAnswer.builder().id(301).examQuestion(examQuestion).build();
        ...
        when(examQuestionRepository.findByExamSessionIdOrderByOrderIndex(1)).thenReturn(List.of(examQuestion));
        when(answerRepository.findByQuestionIdInAndIsCorrectTrue(List.of(11))).thenReturn(List.of(correct1, correct2));
        when(userAnswerRepository.findByExamQuestionExamSessionId(1)).thenReturn(List.of(userAnswer));
    ...
    void getResultsByExam_shouldBuildStatisticsAndRanking() {
        ...
        User u1 = User.builder().id(1).name("A").build();
        User u2 = User.builder().id(2).name("B").build();
        ...
        Result r1 = Result.builder().id(100).examSession(s1).score(9.0f).totalCorrect(9).submittedAt(LocalDateTime.now())
                .build();
        Result r2 = Result.builder().id(101).examSession(s2).score(6.0f).totalCorrect(6).submittedAt(LocalDateTime.now())
                .build();
